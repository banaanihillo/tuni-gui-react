...
// will only see deployed updates on subsequent visits to a page,
// after all the existing tabs open on the page have been closed,
// since previously cached resources are updated in the background.

// To learn more about the benefits of this model
// and instructions on how to opt-in, read https://bit.ly/CRA-PWA
...
export function register(config) {
    if (
        ...
        // The URL constructor is available
        // in all browsers that support SW.
        const publicUrl = new URL(
            process.env.PUBLIC_URL,
            window.location.href
        )
        if (publicUrl.origin !== window.location.origin) {
            // Our service worker won't work
            // if PUBLIC_URL is on a different origin
...
function registerValidServiceWorker(serviceWorkerURL, config) {
    ...
    .then(registration => {
        registration.onupdatefound = () => {
            ...
            // so == null here actually means null?
            // or null || undefined?
...
function checkValidServiceWorker(serviceWorkerURL, config) {
    ...
    // assuming that's supposed to have a comma in between?
    // the omission completely changes the meaning of the sentence
    ...
    .then(response => {
        ...
        if (
            response.status === 404
            || (
                // do you mean null || undefined again?
                contentType != null
                // mixing strict and loose equals, that's amazing
